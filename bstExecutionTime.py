...
"""
    ...
    def maximum(self):
        """Returns the node with the greater key in the subtree rooted by this node."""
        current = self
...
for x in range(10, 500, 10):
    randoms = random.sample(range(100), 100)


    for i in randoms:
        t1 = timeit.timeit('tree.insert(i)', number=1, globals= globals())
        xAxis1 = randoms.index(i) + 1
        insDict[xAxis1] = t1
     #
...
#delete random element
    ...
    deleteRandomDict[x] = t4


plt.plot(insDict.keys(), insDict.values())
plt.plot(getRandomDict.keys(),getRandomDict.values())
plt.plot(getMaxDict.keys(),getMaxDict.values())
plt.plot(deleteRandomDict.keys(),deleteRandomDict.values())
plt.show()
